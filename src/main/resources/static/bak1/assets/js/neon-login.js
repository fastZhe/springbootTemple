...
(function($, window, undefined)
{
    ...
    $(document).ready(function()
    {
        ...
        //$("#kaptchaImage").change(function(){
        //    $("#verificationCode").removeData("previousValue");
        //});
        // Login Form & Validation
        jQuery.validator.addMethod("validateVerify", function(value, element) {
            var flag = 1;
                $.ajax({
                    url : baseurl+"/user/checkVerificationCode.do",
                    data : {verificationCode:$("#verificationCode").val().trim()},
                    type : "POST",
                    dataType : "json",
                    async:false,
                    cache: false,
                ...
                if(flag == 0){
                    ...
                    console.log(1);
                    $("#cuowu").html("验证码错误").css('color','red')
                    get('kaptchaImage').src=baseurl+'/captcha/getCaptchaImage.do?' + Math.floor(Math.random()*100) ;
                    return false;
                }
        ...
        form1.validate({
            ...
            rules: {
                ...
                verificationCode: {
                    ...
                    //    },
                    //    //success:function(){
                    //    //    $("#kaptchaImage").src='${pageContext.request.contextPath}/captcha/getCaptchaImage.do?' + Math.floor(Math.random()*100) ;
                    //    //}
                    //}
                },
            ...
            errorPlacement: function(error, element) { //错误信息位置设置方法
                error.appendTo( element.next("img").next() ); //这里的element是录入数据的对象
            },
            messages:{
                verificationCode:{
                    required:'验证码不能为空'
               ...
               // $(".form-login-error").slideUp('fast');
                ...
                // {
                   ...
                   // neonLogin.setPercentage(40 + random_pct);
                    ...
                    var myurl=baseurl + '/user/checkuser.do'
                    console.log(myurl);
                    $.ajax({
                        url: myurl,
                        method: 'POST',
                        ...
                        data: {
                            ...
                            verificationCode :$("input#verificationCode").val().trim(),
                        ...
                        success: function(response)
                        {
                            ...
                            // {
                                ...
                                {
                                    alert(response.login_message);

                                    get('kaptchaImage').src=baseurl+'/captcha/getCaptchaImage.do?' + Math.floor(Math.random()*100) ;
                                    console.log(response.login_message);
                                ...
                                if(login_status == 'success')
                                {
                                    ...
                                    // {
                                        ...
                                        var redirect_url = baseurl+"/home/index.do";

                                        if(response.redirect_url && response.redirect_url.length)
                                        {
                                            redirect_url = baseurl+response.redirect_url;
                                        }
