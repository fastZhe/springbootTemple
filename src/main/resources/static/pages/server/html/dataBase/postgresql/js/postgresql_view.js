define(['jquery', 'util', 'common', '../../postgresql/js/postgresql_model'], function ($, u, com, _model ) {
    "use strict";
    return function(){
        var model = new _model.Model();
        var arr = model.getInitObj( Object.keys(model.getObj()) );
        com.initModal($('body'), arr, '', {
            title : "第二步：连接信息（PostgreSQL）"
        });
        ...
        $('#modal').on('mousedown','.btn-test,.btn-add', function(e){//这里要使用down事件来设置参数
            ...
            var postgresql = model.getFinalObj( obj );
            $('#modal').data('stepTwo', postgresql);
        });
    };
});