...
define(['jquery', 'util', 'common'], function ($, u, com) {
    function Model() { //数据数据源model
        ...
        Model.prototype.getSignificantObj = function () {
            var obj = {};
            for (var key in this.dqeData) { //只获取有意义的属性
                if (this.dqeData[key] || 0 == this.dqeData[key]) {
                    obj[key] = this.dqeData[key];
                }
            }
            return obj;
        };
        ...
        Model.prototype.getFinalObj = function( obj ) {
        	...
        	if("1010401" == obj.needAuth){
        		step2Data["properties"] =  JSON.stringify({
                    "authInfo" : {
