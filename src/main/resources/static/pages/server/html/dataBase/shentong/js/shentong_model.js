...
define(['jquery', 'util', 'common'], function ($, u, com) {
    function Model() { //数据数据源model
        ...
        Model.prototype.setObj = function (obj) {  //设置对象属性值
            for (var key in obj) {
                this.shentongData[key] = obj[key];
            }
        };
        Model.prototype.getObj = function () {   //获取对象所有属性，空值也获取
            return this.shentongData;
        };
        Model.prototype.getSignificantObj = function () {
            var obj = {};
            for (var key in this.shentongData) { //只获取有意义的属性
                if (this.shentongData[key] || 0 == this.shentongData[key]) {
                    obj[key] = this.shentongData[key];
                }
            }
            return obj;
        };
