...
define(['jquery', 'util', 'common'], function ($, u, com) {
    function Model() { //数据数据源model
        ...
        Model.prototype.setObj = function (obj) {  //设置对象属性值
            for (var key in obj) {
                this.postgresqlData[key] = obj[key];
            }
        };
        ...
        Model.prototype.getSignificantObj = function () {
            var obj = {};
            for (var key in this.postgresqlData) { //只获取有意义的属性
                if (this.postgresqlData[key] || 0 == this.postgresqlData[key]) {
                    obj[key] = this.postgresqlData[key];
                }
            }
            return obj;
        };
