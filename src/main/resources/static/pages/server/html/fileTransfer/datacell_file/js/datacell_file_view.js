define(['jquery', 'util', 'common', '../../datacell_file/js/datacell_file_model'], function ($, u, com, _model ) {
    "use strict";
    return function(){
        var model = new _model.Model();
        var arr = model.getInitObj( Object.keys(model.getObj()) );
        com.initModal($('body'), arr, '', {
            title : "第二步：连接信息（dataCell文件传输）"
        });
        $('#_modal_needAuth').val('1010402').trigger('change').attr('disabled','disabled');
        $('#_modal_authType').val('1010801').trigger('change').attr('display','disabled');
